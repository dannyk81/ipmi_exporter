...
func ipmiMonitoringOutput(rmcp *rmcpConfig) ([]byte, error) {
	return freeipmiOutput("ipmimonitoring", rmcp, "-Q", "--comma-separated-output", "--no-header-output", "--sdr-cache-recreate", "--session-timeout=10000")
}

func ipmiDCMIOutput(rmcp *rmcpConfig) ([]byte, error) {
	return freeipmiOutput("ipmi-dcmi", rmcp, "--get-system-power-statistics", "--session-timeout=10000")
}

func bmcInfoOutput(rmcp *rmcpConfig) ([]byte, error) {
	return freeipmiOutput("bmc-info", rmcp, "--get-device-id", "--session-timeout=10000")
}
